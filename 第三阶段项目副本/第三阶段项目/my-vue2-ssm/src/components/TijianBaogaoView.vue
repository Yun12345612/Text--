  ...
  <div class="examination-result-container">
    ...
    <div class="table-container">
      ...
      >
        ...
        <el-table-column prop="orderNo" label="订单编号" align="center" min-width="150" />
      ...
      <div v-if="currentRecord" class="summary-content">
        <div class="user-info">
          ...
          <el-descriptions :column="2" border>
            ...
            <el-descriptions-item label="年龄">{{ currentRecord.userAge }}岁</el-descriptions-item>
            <el-descriptions-item label="性别">
              {{ currentRecord.userGender === "1" ? "男" : "女" }}
            </el-descriptions-item>
            <el-descriptions-item label="手机号">{{ currentRecord.userPhone }}</el-descriptions-item>
            <el-descriptions-item label="身份证号">{{ currentRecord.userCard }}</el-descriptions-item>
          </el-descriptions>
        </div>
        ...
        <div class="summary-details" style="margin-top: 20px">
          ...
          <div v-if="summaryDetails.length > 0">
            <el-table :data="summaryDetails" border style="width: 100%">
              <el-table-column type="index" label="序号" width="60" align="center" />
              <el-table-column prop="detailName" label="细项名称" align="center" min-width="120" />
              <el-table-column prop="checkResult" label="检查结果" align="center" width="100" />
              <el-table-column prop="resultDescription" label="结果说明" align="center" min-width="150" />
              <el-table-column prop="doctorName" label="检查医生" align="center" width="100" />
      ...
      <div v-if="currentRecord" class="conclusion-content">
        <div class="user-info">
          ...
          <el-descriptions :column="2" border>
            ...
            <el-descriptions-item label="手机号">{{ currentRecord.userPhone }}</el-descriptions-item>
            <el-descriptions-item label="身份证号">{{ currentRecord.userCard }}</el-descriptions-item>
            <el-descriptions-item label="订单类型">{{ currentRecord.orderType }}</el-descriptions-item>
          </el-descriptions>
        </div>
        ...
        <div class="conclusion-text" style="margin-top: 20px">
          ...
          <div class="readonly-content">
            {{ currentRecord.conclusion || "暂无体检总结内容" }}
          </div>
        </div>
      </div>

      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="conclusionDialogVisible = false">关闭</el-button>
      </div>
    </el-dialog>
    ...
    >
      <div v-if="currentRecord" class="report-content">
        <div class="report-header" style="text-align: center; margin-bottom: 20px">
          <h2>体检总报告</h2>
          <p style="color: #666; margin-top: 10px;">订单编号：{{ currentRecord.orderNo }}</p>
        </div>
        ...
        <div class="report-body">
          ...
          <div style="margin-top: 20px">
            <h3>报告内容</h3>
            <div class="readonly-content">
              {{ currentRecord.reportContent || "暂无体检报告内容" }}
            </div>
          </div>
        </div>
      </div>
...
import { getExaminationDetailsByOrder } from '@/api/examinationDetailSummary.js';
    ...
    async handleSummary(row) {
      ...
      try {
        ...
        if (response && response.success) {
          ...
          this.summaryDetails = [];
          Message.warning('该订单暂无体检小结数据');
        }
      } catch (error) {
        console.error('查询小结失败:', error);
        this.summaryDetails = [];
        Message.error('查询小结数据失败');
      }
    },
